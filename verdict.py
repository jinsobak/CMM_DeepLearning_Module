...
def make_predictions(model, input_data):                                                                                # 모델을 사용하여 예측 수행
    predictions = model.predict(input_data.reshape(-1, input_data.shape[0]))                                            # 1차원 배열로 변환 후 예측 수행
    return predictions

csv_file_path ='C:\\Users\\pc\\Desktop\\4학년1학기\\SW_project\\CMM_analysis\\output\\231005_일상검사_야_초_1-5-1_NG.csv' # 데이터 파일 경로
...
predictions = make_predictions(loaded_model, X_input)                                                                   # 예측 수행
