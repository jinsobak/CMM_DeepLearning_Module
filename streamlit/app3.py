####  cmd에서 이렇게 넣어서 실행해야함  streamlit run c:/git_folder/CMM_DeepLearning_Module/streamlit/app3.py
...
def prepare_data(csv_file):
    ...
    return X_train_scaled, X_test_scaled, y_train, y_test, scaler, numeric_features.columns.drop('품질상태')
...
if uploaded_file is not None:
    # 데이터 전처리
    X_train, X_test, y_train, y_test, scaler, numeric_features_columns = prepare_data(uploaded_file)
...
# 첫 번째 CSV 파일이 업로드되었을 때
if uploaded_file is not None:
    # 데이터 전처리
    X_train, X_test, y_train, y_test, scaler = prepare_data(uploaded_file)
    
    # 'numeric_features_columns' 정의
    all_data = pd.read_csv(uploaded_file, encoding='cp949')
    numeric_features_columns = all_data.select_dtypes(include=[np.number]).columns.tolist()  # 숫자형 열 이름 저장
    ...
    if new_file is not None:
        ...
        try:
            ...
            st.error("새로운 CSV 파일의 열 이름이 첫 번째 CSV 파일과 일치하지 않습니다. 올바른 파일을 업로드해주세요.")
        else:
            ...
            # 중복 제거된 이상치 인덱스 출력
            st.write("**새로운 데이터의 이상치 인덱스:**")
            st.write(np.unique(new_anomalies))
else:
    st.write("CSV 파일 업로드")