...
def objective(trial):
    ...
    learning_rate = trial.suggest_float('learning_rate', 1e-5, 1e-1, log=True)  # 학습률
    ...
    X_train, X_test, X_val, y_train, y_test, Y_val, scaler, feature_columns = prepare_data(
        'C:\\Users\\freeman\\Desktop\\빅브라더\\MLP&ML\\datas\\data_jd_hd_delete_material_no_NTC_pca_component_7.csv')
...
def main():
    ...
    study.optimize(objective, n_trials=300)  # 300번의 시도 진행
    ...
    # 모델 학습
    early_stopping = EarlyStopping(min_delta=0.001, patience=10, restore_best_weights=True)
    model.fit(train_dataset, epochs=best_epochs, validation_data=test_dataset, callbacks=[early_stopping])
