...
def performStandScalar(df_datas):
...
def make_pca_dataFrame(data_scaled_base, data_scaled, data_target, data_fileName, num_components):
    #PCA진행
    ...
    df_pca_column_names = [f'component_{i}' for i in range(0, num_pca_components)]
...
if __name__=="__main__":
    ...
    #정규화
    df_scaled = performStandScalar(df_datas=df_datas)
    
    num_components = 4
    df_pca = make_pca_dataFrame(data_scaled_base=df_scaled, data_scaled=df_scaled, data_target=df_target, data_fileName=df_fileName, num_components=num_components)
