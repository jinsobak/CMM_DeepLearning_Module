...
class testClassifier:
    ...
    def build_model(self):
        ...
        hidden_layer1 = tf.keras.layers.Dense(units = 8, activation=activation_func_relu)(input_layer)
        ...
        hidden_layer4 = tf.keras.layers.Dense(units = 8, activation=activation_func_relu)(hidden_layer3)
        hidden_layer5 = tf.keras.layers.Dense(units = 8, activation=activation_func_relu)(hidden_layer4)
        
        activation_func_sig = tf.keras.activations.sigmoid
        output_layer = tf.keras.layers.Dense(units=self.output_dim, activation=activation_func_sig)(hidden_layer5)
        classifier_model = tf.keras.Model(inputs=input_layer, outputs=output_layer)

        print(classifier_model.summary())

        optimize_alg = tf.keras.optimizers.Adam(learning_rate=0.0007)
...
if __name__ == "__main__":
    ...
    log_dir = os.getcwd() + "\\MLP&ML\\log\\" + model_name
    #tensorboard --logdir=./MLP&ML/log --port=6006
